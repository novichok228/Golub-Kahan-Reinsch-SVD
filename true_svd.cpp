...
public:
	...
	BaseMatrix<Type> get_diag(int k) const//работает с конца
	{
		BaseMatrix<Type> res(k, k);
		int c = 0, d = 0;
		bool flag = false;
		for (int i = 0; i < height; ++i) {
			c = 0;
	...
	BaseMatrix<Type> clear_firsts(int k) const
	{
	...
	BaseMatrix<Type> clear_last_row(int k) const
	{
	...
	BaseMatrix<Type> clear_lasts(int k) const {
		BaseMatrix<Type> res(height - k, width - k);
		for (int i = 0; i < height - k; ++i) {
	...
	BaseMatrix<Type> expand_last_row(int k) const {
		BaseMatrix<Type> res(height + k, width);
		for (int i = 0; i < height; ++i) {
			for (int j = 0; j < width; ++j) {
				res.ptr[i][j] = ptr[i][j];
